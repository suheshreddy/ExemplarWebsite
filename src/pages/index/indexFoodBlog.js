...
export default function IndexFoodBlog() {
  ...
  let handleMovePrev = () => {
    setCurrentImageIndex(
      (prevIndex) => (prevIndex + foodImage.length - 1) % foodImage.length
    );
  };
                      ...
                      <div className="group relative overflow-hidden">
                        ...
                        <div className="absolute bottom-0 start-0 end-0 p-6 -mb-96 group-hover:mb-0 duration-500 ease-in-out">
                          <div className="text-center">
                            <Link className="bg-indigo-600 text-white text-xs font-semibold px-2.5 py-0.5 rounded w-fit mx-auto h-5">
                              {item.title}
                            </Link>
                            <Link
                              to="/food-recipe"
                              className="text-white font-semibold hover:text-indigo-600 block text-lg mt-4 duration-500 ease-in-out"
                            >
                              {item.description}
                            </Link>
                            ...
                            <div className="flex items-center justify-center mt-4">
                              <img
                                src={item.imageList}
                                className="size-10 rounded-full shadow-md dark:shadow-gray-800"
                                alt=""
                              />
                              <Link className="font-medium text-white block ms-2">
                                {item.name}
                              </Link>
                            </div>
                          </div>
                        ...
                        <div className="absolute end-0 top-0 p-4 opacity-0 group-hover:opacity-100 duration-500 ease-in-out">
                          <Link
                            to="/#!"
                            className="size-9 inline-flex items-center justify-center tracking-wide align-middle transition duration-500 ease-in-out text-center text-lg bg-white dark:bg-slate-900 border-0 shadow dark:shadow-gray-800 rounded-full text-red-600/20 hover:text-red-600 focus:text-red-600"
                          >
          ...
          <div className="grid lg:grid-cols-5 md:grid-cols-3 grid-cols-2 gap-4 justify-center">
            {foodAbout.map((item, index) => {
              return (
                <div
                  key={index}
                  className="group relative rounded-md shadow-md dark:shadow-gray-800 overflow-hidden"
                >
                  <img
                    src={item.image}
                    className="group-hover:scale-105 duration-500 ease-in-out"
                  ...
                  <div className="absolute top-1/2 -translate-y-1/2 start-0 end-0 text-center">
                    <Link
                      to="/#!"
                      className="text-white font-semibold text-xl mb-0"
                    >
                      {item.title}
                    </Link>
                  </div>
        ...
        <div className="container relative md:mt-24 mt-16">
          <div className="grid grid-cols-1 pb-8 text-center">
            ...
            <p className="text-slate-400 max-w-xl mx-auto">
              Write your own food blog with Exemplar Info that can provide
              everything you need to blog, news, article, etc.
            </p>
          </div>
          <ResponsiveMasonry
                ...
                return (
                  <div className="picture-item p-2 pb-5" key={index}>
                    <div className="group relative">
                      ...
                      <div className="p-3">
                        <Link
                          to={`/food-recipe/${item.id}`}
                          className="block hover:text-indigo-600 text-lg font-semibold"
                        >
                          {item.title}
                        </Link>
                        ...
                        <ul className="list-none space-x-1 flex items-center justify-between mt-2">
                          ...
                          <li className="">
                            <Link
                              to="/#!"
                              className="text-red-600/20 hover:text-red-600 focus:text-red-600"
                            >
                              <i className="mdi mdi-heart"></i>
                            </Link>{' '}
              ...
              <div className="absolute bottom-0 start-0 end-0 p-6 -mb-96 group-hover:mb-0 duration-500 ease-in-out">
                <div className="text-center">
                  ...
                  <Link
                    ...
                    className="text-white font-semibold hover:text-indigo-600 block text-lg mt-4 duration-500 ease-in-out"
                  >
                    Spicy Cauliflower Burgers and Salad Recipe
                  </Link>
              ...
              <div className="grid grid-cols-1 gap-4">
                <div className="group md:flex items-center relative overflow-hidden">
                  ...
                  <div className="md:w-[60%] md:ps-4 pt-4 md:pt-0">
                    ...
                    <Link
                      to="/food-recipe"
                      className="block hover:text-indigo-600 text-lg font-semibold mt-3"
                    >
                      Change Your Strategy: Find a Business Consultant
                    </Link>

                    <span className="text-slate-400 items-center flex mt-3">
                      <MdOutlineCalendarMonth /> 14th July 2022
                    </span>
                  </div>
                </div>
                ...
                <div className="group md:flex items-center relative overflow-hidden">
                  ...
                  <div className="md:w-[60%] md:ps-4 pt-4 md:pt-0">
                    ...
                    <span className="text-slate-400 flex items-center mt-3">
                      <MdOutlineCalendarMonth /> 14th July 2022
                    </span>
                  </div>
                </div>
        ...
        <div className="grid grid-cols-1 relative">
          ...
          {lightBox && (
            <Lightbox
              ...
              prevSrc={
                foodImage[
                  (currentImageIndex + foodImage.length - 1) % foodImage.length
                ]
              }
