...
import { jobAbout } from '../../data/dataFive';

export default function PageJobCompanyDetail() {
    const params = useParams();
    const id = params.id
    const data = jobGrid.find((company) => company.id === parseInt(id))
    return (
        <>
            <Navbar navClass="nav-light" />

            <section className="relative table w-full py-72 bg-[url('../../assets/images/job/job.jpg')] bg-no-repeat bg-center bg-cover">
                    ...
                    <div className="grid md:grid-cols-12 grid-cols-1 gap-[30px]">
                            ...
                            <div className="lg:-mt-[330px] -mt-[299px]">
                                <div className="rounded-md bg-white dark:bg-slate-900 shadow dark:shadow-gray-800">
                                    <div className="text-center py-8 p-6 border-b border-gray-100 dark:border-gray-700">
                                        <img src={data?.image ? data?.image : circle_logo} className="size-24 p-4 shadow-md mx-auto rounded-full block" alt="" />
                                        <h5 className="mt-5 text-xl font-semibold mb-0">{data?.name ? data?.name : 'CircleCi'}</h5>
                                    ...
                                    <div className="p-6">
                                        ...
                                        <ul className="list-none mt-4">
                                            <li className="flex mt-2 items-center font-medium"><Icon.MapPin className="size-4 text-amber-500 me-3"></Icon.MapPin><span className="text-slate-400 me-3">Location :</span> {data?.place ? data?.place : 'San Francisco'}</li>
                                            <li className="flex mt-2 items-center font-medium"><Icon.Link className="size-4 text-amber-500 me-3"></Icon.Link><span className="text-slate-400 me-3">Comapny :</span> {data?.company ? data?.company : 'circleci.com'}</li>
                        ...
                        <div className="lg:col-span-8 md:col-span-7">
                            ...
                            <div className="grid lg:grid-cols-2 grid-cols-1 mt-8 gap-[30px]">
                                {jobAbout.map((item,index)=>{
                                    const Icon = item.icon
                                    return(
                                        <div key={index}>
                                        <Link to="/page-job-detail" className="group flex  bg-white dark:bg-slate-900 transition-all duration-500 hover:scale-105 shadow dark:shadow-gray-800 hover:shadow-md dark:hover:shadow-gray-700 ease-in-out items-center p-3 rounded-md">
